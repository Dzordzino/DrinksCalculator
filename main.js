(function() {
    ...
    function addDrink() {
      ...
      if (inputValue !== "") {
        singleDrink = `<p class="js-orderItem js-drinkOrdered orderButton">${inputValue}<span>${
          price.value
        }</span></p>`;
      ...
      [].forEach.call(allOrders, function(item) {
        ...
        if (info.length > 1) {
          receiptHolder.innerHTML += `<p class="js-listItem js-orderInfo" data-id="${
            info[0]
          }">${info[0]}<span>${info[1]}/${info[2]}</span></p>`;
    ...
    function removeDrink(e) {
      var singleItem = e.currentTarget,
        ...
        totalPrice = orderData.totalPrice,
        listArray = [...parentList.querySelectorAll(".js-orderItem")].filter(
          item => item !== singleItem
        );

      if (drinkParent === "orderList") {
        totalPrice.value =
          totalPrice.value - Number(singleItem.querySelector("span").innerHTML);
      }
    ...
    function renderSumData(completeSum) {
      ...
      [].forEach.call(completeSum, function(item) {
        ...
        sumHolder.innerHTML += `<p class="orderButton">${singleItem[1]}<span>${
          item[1]
        }</span></p>`;
      });
    }

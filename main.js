(function() {
    ...
    function addEventListeners() {
        eventHandler(".js-enterDrink", "click", addDrink);
    ...
    function addDrink() {
        ...
        if (inputValue != "") {
            singleDrink = `<p class="js-orderItem js-drinkOrdered orderButton">${inputValue}<span>${price.value}</span></p>`;
            orderData.drinksList.innerHTML += singleDrink;
            drinksArray.push(singleDrink);
            orderData.setItem("drinks", drinksArray);
            // eventHandler(".js-listItem", "click", itemClick);
            input.value = "";
            price.value = "";
        }
    ...
    // function itemClick(e) {
        ...
        //     listArray = [...document.querySelectorAll(".js-listItem")].filter(item => item != singleItem),
        //     storageArray = [];

        // orderData.drinksList.innerHTML = "";
        // [].forEach.call(listArray, function(item) {
        //     document.querySelector(".js-buttonsHolder").appendChild(item);
        // });
        // orderData.setItem("drinks", storageArray);
    // }
    ...
    function renderReceipts() {
        ...
        [].forEach.call(allOrders, function(item) {
            info = item.split("&");
            if (info.length > 1) {
                receiptHolder.innerHTML += `<p class="js-listItem js-orderInfo" data-id="${info[0]}">${info[0]}<span>${info[1]}/${info[2]}</span></p>`;
    ...
    function removeDrink(e) {
        var singleItem = e.currentTarget,
            ...
            totalPrice = orderData.totalPrice,
            listArray = [...parentList.querySelectorAll(".js-orderItem")].filter(item => item != singleItem);
    ...
    function savePlace() {
        ...
        if (placeName !== "" && drinkList.length !== 0) {
            ...
            if (!allPlaces.includes(placeName)) {
    ...
    function drinksSum(sum, array) {
        ...
        [].forEach.call(sum, function(item) {
            ...
            itemSpan = spanElement[1];
            singleDrink = item.replace(spanElement[0], "");
            array.push(singleDrink);
        });
        sumAll = array.reduce((acc, it) => {
            acc[it] = acc[it] + 1 || 1;
            return acc;
        }, {});

        renderSumData(Object.entries(sumAll));
    }

    function renderSumData(completeSum) {
        var sumHolder = document.querySelector(".js-sumInfo"),
            singleItem = "";

        sumHolder.innerHTML = "";
        [].forEach.call(completeSum, function(item) {
            singleItem = item[0].match('<\s*p[^>]*>(.*?)<\s*/\s*p>');
            sumHolder.innerHTML += `<p class="orderButton">${singleItem[1]}<span>${item[1]}</span></p>`;
        });
    }
